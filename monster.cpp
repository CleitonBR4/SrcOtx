	...
	Creature()
{
	...
	for (const auto& it : g_creatureEvents->getEvents()) {
		if (it->isLoaded()) {
			const std::string& autoRegister = it->getAutoRegister();
			if (autoRegister == "monsters" || autoRegister == "both") {
				registerCreatureEvent(it->getName());
			}
		}
	}
}
	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	else
	{
		...
		// --- INÍCIO DA CORREÇÃO DE DEEP SLEEP (STAIRHOP) ---
		// Força o monstro a "redescobrir" o alvo se ele acabou de pisar no mesmo andar (Z)
		else if(canSeeNewPos && newPos.z == getPosition().z && oldPos.z != getPosition().z)
			onCreatureEnter(const_cast<Creature*>(creature));
		// --- FIM DA CORREÇÃO ---
...
void Monster::onCreatureLeave(Creature* creature)
{
	...
	if(isOpponent(creature))
	{
		...
		if(it != targetList.end())
		{
			...
			// --- INÍCIO DA OTIMIZAÇÃO: DESAPEGO RÁPIDO ---
			// Se o jogador saiu da tela (deixou o range de visão), o monstro
			// para de tentar calcular rotas impossíveis e limpa o alvo principal
...
void Monster::updateIdleStatus()
{
	...
	if(conditions.empty())
	{
		...
		{
			// --- INÍCIO DA OTIMIZAÇÃO: DEEP SLEEP (EIXO Z) ---
			idle = true; // Assume que deve dormir por padrão
			for(CreatureList::iterator it = targetList.begin(); it != targetList.end(); ++it)
			{
				// Se houver PELO MENOS UM alvo no mesmo andar, ele acorda!
				if((*it)->getPosition().z == getPosition().z)
				{
					idle = false;
					break;
				}
