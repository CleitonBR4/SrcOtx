...
ReturnValue Spells::onPlayerSay(Player* player, const std::string& words)
{
	...
	if(!g_config.getBool(ConfigManager::SPELL_NAME_INSTEAD_WORDS))
	{
		...
		return g_game.internalCreatureSay(player, type, reWords, (player->isGhost() || player->isInvisible())) ?
	...
	return g_game.internalCreatureSay(player, type, ret, (player->isGhost() || player->isInvisible()),
...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	if(readXMLString(p, "name", strValue))
	{
		...
		{
			"melee", "physical", "poison", "earth", "fire", "ice", "freeze", "energy", "drown", "death", "curse", "fuuton",
...
bool InstantSpell::Levitate(const InstantSpell*, Creature* creature, const std::string& param)
{
	...
	if(ret == RET_NOERROR)
	{
		g_game.addMagicEffect(player->getPosition(), MAGIC_EFFECT_TELEPORT, player);
		return true;
	}

	player->sendCancelMessage(ret);
	g_game.addMagicEffect(player->getPosition(), MAGIC_EFFECT_POFF, player);
	return false;
}
