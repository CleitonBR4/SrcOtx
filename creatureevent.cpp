...
bool CreatureEvents::playerLogin(Player* player)
{
	...
	for (auto& it : m_creatureEvents) {
		if (it->isLoaded()) {
			CreatureEventType_t eventType = it->getEventType();
			if (eventType == CREATURE_EVENT_LOGIN) {
				if (!it->executePlayer(player)) {
					result = false;
				}
			} else if (eventType != CREATURE_EVENT_LOGOUT) {
				const std::string& autoRegister = it->getAutoRegister();
				if (autoRegister == "players" || autoRegister == "both") {
					player->registerCreatureEvent(it->getName());
				}
			}
		}
	}
...
bool CreatureEvent::configureEvent(xmlNodePtr p)
{
	...
	if (readXMLString(p, "autoregister", strValue)) {
		m_autoRegister = asLowerCaseString(strValue);
	}
...
void CreatureEvent::executeDeath(Creature* creature, Item* corpse, DeathList deathList)
{
	...
	if (m_interface->reserveEnv()) {
		...
		if (m_scripted == EVENT_SCRIPT_BUFFER) {
			...
			m_interface->loadBuffer(scriptstream.str());
			m_interface->releaseEnv();
		} else {
			#ifdef __DEBUG_LUASCRIPTS__
			char desc[35];
