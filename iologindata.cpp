...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	<< "`save`, `reset`, `HPbonus`, `MPbonus`, `r_points` FROM `players` WHERE "
	...
	player->resets = result->getDataInt("reset");
	player->hpBonus = result->getDataInt("HPbonus");
	player->mpBonus = result->getDataInt("MPbonus");
	player->rPoints = result->getDataInt("r_points");
	...
	// load auto loot
	query.str("");
	query << "SELECT `item_id` FROM `player_autoloot` WHERE `player_id` = " << player->getGUID();
	if ((result = db->storeQuery(query.str()))) {
		do {
			player->addAutoLootItemId(result->getDataInt("item_id"));
		} while (result->next());
	}
	return true;
}
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	query << "`reset` = " << player->resets << ", ";
	query << "`HPbonus` = " << player->hpBonus << ", ";
	query << "`MPbonus` = " << player->mpBonus << ", ";
	query << "`r_points` = " << player->rPoints << ", ";
	...
	query.str("");
	stmt.setQuery("INSERT INTO `player_autoloot` (`player_id`, `item_id`) VALUES ");
	for (uint16_t itemId : player->autoLoot) {
		query << player->getGUID() << ',' << itemId;
		if (!stmt.addRow(query)) {
			return false;
		}
	}
