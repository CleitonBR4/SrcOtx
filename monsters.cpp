...
void MonsterType::dropLoot(Container* corpse)
{
	...
	if(owner->getParty() && message > LOOTMSG_PLAYER)
		owner->getParty()->broadcastPartyLoot(MSG_CHANNEL, ss.str());
	else if (message == LOOTMSG_PLAYER || message == LOOTMSG_BOTH) {
		uint16_t lootChannelId = g_config.getNumber(ConfigManager::LOOT_CHANNEL);
		ChatChannel* lootChannel = g_chat.getChannel(owner, lootChannelId);
		if (lootChannel && lootChannel->hasUser(owner)) {
			owner->sendChannelMessage(nick, ss.str(), MSG_CHANNEL, lootChannelId);
		} else {
...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	else
	{
		...
		if(tmpName == "melee" || tmpName == "distance")
		{
			...
			else if(readXMLInteger(node, "dazzle", intValue) || readXMLInteger(node, "fuuton", intValue))
			{
		...
		else if(tmpName == "skills" || tmpName == "attributes")
		{
			...
			tmpName == "fuutoncondition" || tmpName == "dazzlecondition" ||
			tmpName == "drowncondition")
		{
			...
			else if(tmpName == "fuutoncondition" || tmpName == "dazzlecondition")
			{
				conditionType = CONDITION_DAZZLED;
				tickInterval = 10000;
			}
