...
void Creature::onThink(uint32_t interval)
{
	...
	if(followCreature)
	{
		...
		{
			// --- INÍCIO DA OTIMIZAÇÃO: FILTRO DE RECALCULO (ONTHINK) ---
			bool canUpdate = true;
			...
			{
				// Checa se já passou 1 segundo (1000ms) desde a última falha de rota do monstro.
				if(OTSYS_TIME() - lastFailedFollow <= 100)
					canUpdate = false;
			}

			// O monstro só ganha permissão para pensar na rota se o cooldown tiver acabado
			if(canUpdate)
			{
				walkUpdateTicks = 0;
				forceUpdateFollowPath = false;
			...
			// Nota: Se 'canUpdate' for falso, ele não zera as variáveis. 
			// Ele simplesmente aguarda silenciosamente até o próximo tick onde o cooldown já terá passado.
			// --- FIM DA OTIMIZAÇÃO ---
		}
	}
...
{
	// --- INÍCIO DA OTIMIZAÇÃO: ANTI-DRUNK LOOP ---
	// Verifica usando a máscara de bits ultra-rápida (hasCondition) se a criatura está bêbada.
	// Se ela não estiver bêbada (99.9% dos casos para monstros) ou estiver usando
	// um anel anti-drunk (isSuppress), nós abortamos a função imediatamente na primeira linha, 
	// poupando a CPU de rodar o loop de busca em todos os 6.000 passos!
	if(!hasCondition(CONDITION_DRUNK) || isSuppress(CONDITION_DRUNK))
		return;
	// --- FIM DA OTIMIZAÇÃO ---
	...
	g_game.internalCreatureSay(this, MSG_SPEAK_MONSTER_SAY, "Hicks!", (isGhost() || isInvisible(true)));
}
	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if(creature == followCreature || (creature == this && followCreature))
	{
		if(hasFollowPath)
		{
			// --- INÍCIO DA OTIMIZAÇÃO: FIM DO SPAM DE RECALCULO ---
			// Removemos a criação da Task instantânea que inundava o Dispatcher
			// e forçamos o monstro a usar o seu tempo natural de "pensamento" (onThink)
			// para recalcular a rota de forma cadenciada e imune a lag de CPU.
			forceUpdateFollowPath = true;
			// --- FIM DA OTIMIZAÇÃO ---
		}
...
void Creature::dropCorpse(DeathList deathList)
{
	...
	for (auto& it : getCreatureEvents(CREATURE_EVENT_DEATH)) {
...
BlockType_t Creature::blockHit(Creature* attacker, CombatType_t combatType, int32_t& damage,
	bool checkDefense, bool checkArmor, bool reflect, bool field, bool element)
{
	...
	// --- INÍCIO DO SISTEMA DE REFLECT NATIVO ---
	// Note que agora também checamos a variável 'reflect' para evitar loops infinitos
	else if (reflect && damage > 0 && attacker && attacker != this && attacker->getPlayer() && getPlayer())
	{
		std::string storageValue;
		if (getPlayer()->getStorage("11903", storageValue))
		{
			int32_t reflectSkill = atoi(storageValue.c_str());
			
			if (reflectSkill > 0 && random_range(1, 800) <= reflectSkill)
			{
				// Calcula 70% do dano bruto para refletir
				int32_t reflectedDamage = (int32_t)std::ceil(damage * 0.7);
				
				// Efeito Dodge: Zera o dano que a vítima iria tomar
				damage = 0;
				
				// Envia o texto "REFLECT" na vítima
				g_game.addAnimatedText(getPosition(), 16, "REFLECT");
				
				// --- MITIGAÇÃO DO DANO REFLETIDO ---
				int32_t mitigatedReflect = reflectedDamage;
				// Fazemos o atacante testar as próprias defesas contra o dano que ele mesmo causou.
				// O 'false' no final desliga o Reflect do atacante, impedindo o loop infinito!
				attacker->blockHit(this, combatType, mitigatedReflect, true, true, false);

				// Se após o armor/defense o dano ainda for maior que 0
				if (mitigatedReflect > 0)
				{
					// Aplica o dano mitigado no atacante
					attacker->drainHealth(this, combatType, mitigatedReflect);
					
					// Efeito Visual do dano (vermelho) no atacante
					char dmgMsg[32];
					sprintf(dmgMsg, "%d", mitigatedReflect);
					g_game.addAnimatedText(attacker->getPosition(), 180, dmgMsg);
					
					// --- MENSAGENS NO SERVER LOG (COMBAT LOG) ---
					if (Player* attackerPlayer = attacker->getPlayer()) {
						char buffer[256];
						sprintf(buffer, "You were reflected for %d damage by %s.", mitigatedReflect, getName().c_str());
						attackerPlayer->sendTextMessage(MSG_DAMAGE_RECEIVED, buffer);
					}
					
					if (Player* victimPlayer = getPlayer()) {
						char buffer[256];
						sprintf(buffer, "You reflected %d damage to %s.", mitigatedReflect, attacker->getName().c_str());
						victimPlayer->sendTextMessage(MSG_DAMAGE_DEALT, buffer);
					}
				}
	...
	// --- FIM DO SISTEMA DE REFLECT NATIVO ---
