...
void LuaInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "getPlayerHpBonus", LuaInterface::luaGetPlayerHpBonus);
	lua_register(m_luaState, "setPlayerHpBonus", LuaInterface::luaSetPlayerHpBonus);
	lua_register(m_luaState, "getPlayerMpBonus", LuaInterface::luaGetPlayerMpBonus);
	lua_register(m_luaState, "setPlayerMpBonus", LuaInterface::luaSetPlayerMpBonus);
	lua_register(m_luaState, "doPlayerResetLevel", LuaInterface::luaDoPlayerResetLevel);
	// Reset Points
	lua_register(m_luaState, "getPlayerRPoints", LuaInterface::luaGetPlayerRPoints);
	lua_register(m_luaState, "setPlayerRPoints", LuaInterface::luaSetPlayerRPoints);
	...
	//doPlayerSendStats(cid)
	lua_register(m_luaState, "doPlayerSendStats", LuaInterface::luaDoPlayerSendStats);
	...
	// auto loot
	lua_register(m_luaState, "getPlayerAutoLootList", LuaInterface::luaGetPlayerAutoLootList);
	lua_register(m_luaState, "erasePlayerAutoLootList", LuaInterface::luaErasePlayerAutoLootList);
	lua_register(m_luaState, "removePlayerAutoLootItem", LuaInterface::luaRemovePlayerAutoLootItem);
	lua_register(m_luaState, "addPlayerAutoLootItem", LuaInterface::luaAddPlayerAutoLootItem);
	lua_register(m_luaState, "getContainerFreeSlots", LuaInterface::luaGetContainerFreeSlots);
	lua_register(m_luaState, "doCloneItem", LuaInterface::luaDoCloneItem);
}

// auto loot
int32_t LuaInterface::luaGetPlayerAutoLootList(lua_State* L)
{
	//getPlayerAutoLootList(cid)
	if(Player* player = getEnv()->getPlayerByUID(popNumber(L))) {
		lua_createtable(L, 0, player->autoLoot.size());
		for (const auto& it : player->autoLoot) {
...
int32_t LuaInterface::luaErasePlayerAutoLootList(lua_State* L)
{
	//erasePlayerAutoLootList(cid)
	if(Player* player = getEnv()->getPlayerByUID(popNumber(L))) {
		player->autoLoot.clear();
		lua_pushboolean(L, true);
	} else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushnil(L);
...
int32_t LuaInterface::luaRemovePlayerAutoLootItem(lua_State* L)
{
	//removePlayerAutoLootItem(cid, itemid)
	uint16_t itemId = static_cast<uint16_t>(popNumber(L));
	if(Player* player = getEnv()->getPlayerByUID(popNumber(L)))
		lua_pushboolean(L, player->removeAutoLootItemId(itemId));
	else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushnil(L);
	}
...
int32_t LuaInterface::luaAddPlayerAutoLootItem(lua_State* L)
{
	//addPlayerAutoLootItem(cid, itemid)
	uint16_t itemId = static_cast<uint16_t>(popNumber(L));
	if(Player* player = getEnv()->getPlayerByUID(popNumber(L)))
		lua_pushboolean(L, player->addAutoLootItemId(itemId));
	else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushnil(L);
	}
...
int32_t LuaInterface::luaGetContainerFreeSlots(lua_State* L)
{
	//getContainerFreeSlots(cid)
	if(Container* container = getEnv()->getContainerByUID(popNumber(L)))
		lua_pushnumber(L, container->getFreeSlots());
	else {
...
int32_t LuaInterface::luaDoCloneItem(lua_State* L)
{
	//doCloneItem(uid)
	ScriptEnviroment* env = getEnv();
	if(Item* item = env->getItemByUID(popNumber(L))) {
		...
		if(clonedItem) {
			clonedItem->setParent(VirtualCylinder::virtualCylinder);
			env->addTempItem(env, clonedItem);
			lua_pushnumber(L, env->addThing(clonedItem));
			return 1;
		}
	} else {
...
{
	//getPlayerItemById(cid, deepSearch, itemId[, subType = -1])
	//getPlayerItemById(cid, deepSearch, itemId[, rangeMin = 0[, rangeMax = 100]])
	int32_t rangeMax = 100;
	int32_t subType = -1;

	int params = lua_gettop(L);
	if (params > 4) {
		rangeMax = static_cast<int32_t>(popNumber(L));
	}

	if (params > 3) {
		subType = static_cast<int32_t>(popNumber(L));
	}

	int32_t itemId = static_cast<int32_t>(popNumber(L));
	...
	if (params > 4) {
		if (subType > rangeMax) {
			std::swap(subType, rangeMax);
		}

		item = g_game.findItemOfTypeByRange(player, itemId, deepSearch, subType, rangeMax);
	} else {
		item = g_game.findItemOfType(player, itemId, deepSearch, subType);
	}
...
// ==== R_POINTS SYSTEM NATIVO ====
int32_t LuaInterface::luaGetPlayerRPoints(lua_State* L)
{
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
		lua_pushnumber(L, player->getRPoints());
	else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
	}
...
int32_t LuaInterface::luaSetPlayerRPoints(lua_State* L)
{
	uint32_t rPoints = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
		player->setRPoints(rPoints);
		lua_pushboolean(L, true);
	} else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaDoPlayerSendStats(lua_State* L)
{
    // doPlayerSendStats(cid)
    ScriptEnviroment* env = getEnv();
    if(Player* player = env->getPlayerByUID(popNumber(L)))
...
{
	uint32_t resets = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
		player->setResets(resets);
		lua_pushboolean(L, true);
	} else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaGetPlayerHpBonus(lua_State* L)
{
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
		lua_pushnumber(L, player->getHpBonus());
	else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
	}
...
int32_t LuaInterface::luaSetPlayerHpBonus(lua_State* L)
{
	uint32_t hpBonus = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
...
int32_t LuaInterface::luaGetPlayerMpBonus(lua_State* L)
{
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
		lua_pushnumber(L, player->getMpBonus());
	else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
	}
...
int32_t LuaInterface::luaSetPlayerMpBonus(lua_State* L)
{
	uint32_t mpBonus = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L))) {
...
int32_t LuaInterface::luaDoPlayerResetLevel(lua_State* L)
{
	uint64_t newExp = popNumber(L);
