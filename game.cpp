...
Item* Game::findItemOfTypeByRange(Cylinder* cylinder, uint16_t itemId,
	bool depthSearch /*= true*/, int32_t rangeMin /*= 0*/, int32_t rangeMax /*= 100*/)
{
	if (!cylinder) {
		return nullptr;
	}
	...
	for (int32_t i = cylinder->__getFirstIndex(); i < cylinder->__getLastIndex(); ++i) {
		...
		uint16_t itemCount = item->getSubType();
		if (item->getID() == itemId && itemCount >= rangeMin && itemCount <= rangeMax) {
			return item;
		} else {
			if (depthSearch) {
				if (Container* subContainer = item->getContainer()) {
					containersVec.push_back(subContainer);
				}
			}
		}
	}
	...
	size_t i = 0;
	while (i < containersVec.size()) {
		Container* container = containersVec[i++];
	...
	int32_t& healthChange, const bool& checkDefense, const bool& checkArmor, const bool& field/* = false*/, const bool& element/* = false*/)
{
	...
	{
		if(!element)
			addMagicEffect(list, targetPos, MAGIC_EFFECT_POFF, target);
...
bool Game::combatChangeHealth(const CombatParams& params, Creature* attacker, Creature* target, int32_t healthChange, const bool& force)
{
	...
	if(healthChange > 0)
	{
		...
		if(oldHealth != target->getHealth() && g_config.getBool(ConfigManager::SHOW_HEALTH_CHANGE)
			&& !target->isGhost() && !target->isInvisible(true)
	...
	const CombatType_t& combatType/* = COMBAT_MANADRAIN*/, const bool& inherited/* = false*/)
{
	...
	if(manaChange > 0)
	{
		...
		if(oldMana != target->getMana() && g_config.getBool(ConfigManager::SHOW_MANA_CHANGE)
			&& !target->isGhost() && !target->isInvisible(true)
	...
	const std::string& text)
{
	...
	void Game::addMagicEffect(const Position& pos, const uint16_t& effect, Creature* sender/*= nullptr*/)
	{
		SpectatorVec list;
		map->getSpectators(list, pos, true, true);
		addMagicEffect(list, pos, effect, sender);
	}
		...
		Creature* sender/*= nullptr*/)
	{
		...
		if (senderPlayer && (senderPlayer->isInvisible()
			|| (senderPlayer->isGhost() && !g_config.getBool(ConfigManager::GHOST_SPELL_EFFECTS)))) {
			senderPlayer->sendMagicEffect(pos, effect);
			return;
		}
	}
...
#else
	void Game::addMagicEffect(const Position& pos, const uint8_t& effect, Creature* sender/*= nullptr*/)
	{
		SpectatorVec list;
		map->getSpectators(list, pos, true, true);
		addMagicEffect(list, pos, effect, sender);
