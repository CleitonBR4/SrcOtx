	...
	uint32_t flags, Creature* actor/* = NULL*/) const
{
	...
	// >>> OTX2 FIX: Matryoshka Fix e Prevenção de loop <<<
	if(const Container* container = item->getContainer())
	{
		// 1. Checagem nativa: Impede colocar a mochila dentro de si mesma
		...
		// 2. Nossa nova checagem: Limite de profundidade absoluta (Depth Limit)
		uint32_t depth = 0;
		const Cylinder* currentParent = this; // Começamos do container atual
		
		while(currentParent) 
		{
			depth++;
			currentParent = currentParent->getParent(); // Sobe um nível na árvore
		}

		// Hierarquia normal máxima esperada: BP -> BP -> BP -> BP -> Player -> Tile (Profundidade 6)
		// Se a profundidade for 8 ou maior, o sistema cancela a ação silenciosamente.
		if(depth >= 10) 
		{
			return RET_NOTPOSSIBLE; 
		}
...
int64_t Container::getFreeSlots()
{
	int64_t freeSlots = std::max<int64_t>(static_cast<int64_t>(capacity()) - size(), 0);
	for (Item* item : itemlist) {
		if (Container* subContainer = item->getContainer()) {
			freeSlots += subContainer->getFreeSlots();
		}
	}
	return freeSlots;
}