...
int32_t Player::getCriticalHitChance() const
{
	...
	for(; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
		{
			...
			if (slotPosition > 0) {
				uint32_t currentSlotBit = (1 << (i - 1));
				bool valid = (slotPosition & currentSlotBit) != 0;
				// Garante suporte a armas de duas mãos
				if (!valid && (slotPosition & SLOTP_TWO_HAND) && (currentSlotBit == SLOTP_LEFT || currentSlotBit == SLOTP_RIGHT)) {
					valid = true;
				}
				if (!valid) continue;
			} else {
				WeaponType_t weaponType = item->getWeaponType();
				if (weaponType == WEAPON_AMMO && i != SLOT_AMMO) continue;
				if (weaponType != WEAPON_AMMO && weaponType != WEAPON_NONE && i != SLOT_LEFT && i != SLOT_RIGHT) continue;
			}
...
int32_t Player::getPlayerInfo(playerinfo_t playerinfo) const
{
	switch(playerinfo)
	{
		...
		{
			int32_t maxML = (int32_t)magLevel + varStats[STAT_MAGICLEVEL];
			int32_t slotBonus = 0;
			for(int32_t i = SLOT_FIRST; i <= SLOT_LAST; ++i) {
				if(Item* item = getInventoryItem((slots_t)i)) {
					bool ok = false;
					int32_t val = item->getIntegerAttribute("slot_ml", ok);
					if(ok) slotBonus += val;
				}
			}
			if(slotBonus > 0)
				maxML += (int32_t)(magLevel * (slotBonus / 100.0));
				
			return std::max((int32_t)0, maxML);
		}
		...
		{
			int32_t maxH = (int32_t)healthMax + varStats[STAT_MAXHEALTH];
			int32_t slotBonus = 0;
			for(int32_t i = SLOT_FIRST; i <= SLOT_LAST; ++i) {
				if(Item* item = getInventoryItem((slots_t)i)) {
					bool ok = false;
					int32_t val = item->getIntegerAttribute("slot_hp", ok);
					if(ok) slotBonus += val;
				}
			}
			if(slotBonus > 0)
				maxH += (int32_t)(healthMax * (slotBonus / 100.0));
				
			return std::max((int32_t)1, maxH);
		}
		case PLAYERINFO_MANA:
			return mana;
		case PLAYERINFO_MAXMANA:
		{
			int32_t maxM = (int32_t)manaMax + varStats[STAT_MAXMANA];
			int32_t slotBonus = 0;
			for(int32_t i = SLOT_FIRST; i <= SLOT_LAST; ++i) {
				if(Item* item = getInventoryItem((slots_t)i)) {
					bool ok = false;
					int32_t val = item->getIntegerAttribute("slot_mp", ok);
					if(ok) slotBonus += val;
				}
			}
			if(slotBonus > 0)
				maxM += (int32_t)(manaMax * (slotBonus / 100.0));
				
			return std::max((int32_t)0, maxM);
		}
...
int32_t Player::getDefaultStats(stats_t stat)
{
	switch(stat)
	{
		case STAT_MAGICLEVEL:
			return magLevel;   // Lê a variável base direta, sem itens
		case STAT_MAXHEALTH:
			return healthMax;  // Lê a variável base direta, sem itens
		case STAT_MAXMANA:
			return manaMax;    // Lê a variável base direta, sem itens
		case STAT_SOUL:
			return soul;       // Lê a variável base direta, sem itens
		default:
			break;
	}
...
{
    // --- INÍCIO DA CORREÇÃO DE OVERFLOW DE SLOTS ---
    // Garante que a vida atual não ultrapasse a máxima ao perder um slot
    if (health > getMaxHealth()) {
        health = getMaxHealth();
    }

    // Garante que a mana atual não ultrapasse a máxima ao perder um slot
    if (mana > getMaxMana()) {
        mana = getMaxMana();
    }
    // --- FIM DA CORREÇÃO ---
...
void Player::addExperience(uint64_t exp)
{
	...
	while(experience >= nextLevelExp)
	{
        ...
        // Adicione a verificação do limite máximo de capacidade aqui
        if (capacity > 90000)
        {
            // Limite máximo atingido, ajuste para o valor máximo permitido
            capacity = 90000;
        }
...
{
    int32_t fistLevel = getSkill(SKILL_FIST, SKILL_LEVEL);

    // Fórmula: 1600 menos 100 para cada 10 levels completos
    // Exemplo: Level 35 -> (35 / 10) = 3.  1600 - (3 * 100) = 1300.
    int32_t speed = 1600 - ((fistLevel / 10) * 100);

    // Trava o valor: Máximo de 1500 (mais lento) e Mínimo de 500 (mais rápido)
    speed = std::max((int32_t)500, std::min((int32_t)1500, speed));
...
bool Player::removeAutoLootItemId(uint16_t id)
{
	auto it = std::find(autoLoot.begin(), autoLoot.end(), id);
	if (it != autoLoot.end()) {
		//order is not important here, so delete it faster
		std::iter_swap(it, autoLoot.end() - 1);
		autoLoot.pop_back();
		return true;
	}
	return false;
...
bool Player::addAutoLootItemId(uint16_t id)
{
	auto it = std::find(autoLoot.begin(), autoLoot.end(), id);
	if (it == autoLoot.end()) {
