...
class Player : public Creature, public Cylinder
{
	public:
		...
		// R_Points System
		uint32_t rPoints = 0;
		uint32_t getRPoints() const { return rPoints; }
		void setRPoints(uint32_t v) { rPoints = v; }

		uint32_t getResets() const { return resets; }
		void setResets(uint32_t v) { resets = v; }
		uint32_t getHpBonus() const { return hpBonus; }
		void setHpBonus(uint32_t v) { hpBonus = v; }
		uint32_t getMpBonus() const { return mpBonus; }
		void setMpBonus(uint32_t v) { mpBonus = v; }
		uint32_t getLevel() const {return level;}
		uint64_t getExperience() const {return experience;}
		void setLevel(uint32_t newLevel) { level = newLevel; }
		void setExperience(uint64_t newExp) { experience = newExp; }
		...
		// auto loot
		std::vector<uint16_t> autoLoot;
		bool removeAutoLootItemId(uint16_t id);
		bool addAutoLootItemId(uint16_t id);
