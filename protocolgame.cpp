	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster)
{
		...
		(accountManager && name == "Account Manager"))
	{
		...
		player->setID();
		#ifdef __UMBYOTS_NO_MAGEBOMB__
    PlayerVector temp;
    temp = g_game.getPlayersByIP(player->getIP());
    if (temp.size() >= 10) {
        disconnectClient(0x14, "Only 10 connections allowed from the same IP.");
        return; // Retorna sem fazer nada (void) se a autenticação falhar
    }
    #endif
