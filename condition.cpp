...
{
	auraEffect = 0xFFFF; // 0xFFFF significa "Nenhum Efeito"
	auraTicks = 0;
	auraInterval = 1000; // Padrão: 1 segundo, caso não defina no script
	auraOffsetX = 0;     // Padrão: 0 (No próprio jogador)
	auraOffsetY = 0;     // Padrão: 0 (No próprio jogador)
	//
}
...
bool Condition::setParam(ConditionParam_t param, int32_t value)
{
	switch(param)
	{
		...
		case CONDITIONPARAM_AURA:
			auraEffect = (uint16_t)value;
			return true;
			
		case CONDITIONPARAM_AURA_TICKS:
			auraInterval = value;
			return true;

		case CONDITIONPARAM_AURA_X:
			auraOffsetX = value;
			return true;

		case CONDITIONPARAM_AURA_Y:
			auraOffsetY = value;
			return true;
...
bool Condition::executeCondition(Creature* creature, int32_t interval)
{
	...
	// === NOVO SISTEMA DE AURA (C++) ===
	if(auraEffect != 0xFFFF) 
	{
		auraTicks -= interval;
		if(auraTicks <= 0)
		{
			// Pega a posição do jogador e aplica os ajustes (Offsets)
			Position pos = creature->getPosition();
			pos.x += auraOffsetX;
			pos.y += auraOffsetY;
			
			g_game.addMagicEffect(pos, auraEffect);
			
			// Usa o seu tempo customizado em vez do "1000" fixo!
			auraTicks = auraInterval; 
		}
	}
	// ==================================
...
void ConditionAttributes::updatePercentSkills(Player* player)
{
	...
	{
		if(skillsPercent[i])
            // Correção: Trocado += por = para evitar o acúmulo fantasma de skills
...
void ConditionAttributes::updatePercentStats(Player* player)
{
	...
	{
		if(statsPercent[i])
            // Correção: Trocado += por = para evitar o acúmulo fantasma de vida/mana
