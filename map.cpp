	...
	const FrozenPathingConditionCall& pathCondition, const FindPathParams& fpp)
{
	...
	while (fpp.maxSearchDist != -1 || nodes.getClosedNodes() < fpp.maxClosedNodes) {
		...
		for (uint_fast32_t i = 0; i < dirCount; ++i) {
			...
			// --- INÍCIO DA OTIMIZAÇÃO: TIE-BREAKER DE DIREÇÃO ---
			int_fast32_t directionPenalty = 0;
			if (n->parent) {
				// Verifica a direção do passo anterior e a direção do próximo passo
				int_fast32_t dx1 = n->x - n->parent->x;
				int_fast32_t dy1 = n->y - n->parent->y;
				int_fast32_t dx2 = pos.x - n->x;
				int_fast32_t dy2 = pos.y - n->y;
				
				// Se a direção mudou (zigue-zague), aplica uma micropenalidade de custo (1)
				// Isso força o algoritmo a priorizar linhas retas, evitando o "Flood-Fill"
				if (dx1 != dx2 || dy1 != dy2) {
					directionPenalty = 1; 
				}
			}
			
			// Somamos a penalidade ao custo total do novo nodo
			const int_fast32_t newf = f + cost + extraCost + directionPenalty;
			// --- FIM DA OTIMIZAÇÃO ---
